...
const advanceTimeTo = async (targetTime) => {
  ...
  await wait(targetTime-time);
}

contract("Farm With deflation token", ([owner, alice, bob, carl]) => {
  before(async () => {
  ...
  before(async () => {
    await Promise.all([
      ...
      this.lp.mint(bob, 1000),
      this.lp.mint(carl, 4000),
    ]);
    this.aliceLpBalance = 10000;
    this.bobLpBalance = 1000;
    this.carlLpBalance = 4000;
    ...
    assert.equal(this.aliceLpBalance, balanceAlice);
    assert.equal(this.bobLpBalance, balanceBob);
    assert.equal(this.carlLpBalance, balanceCarl);
  });

  before(async () => {
    await Promise.all([this.lp2.mint(alice, 2000), this.lp2.mint(carl, 1600)]);
    ...
    this.aliceLp2Balance = 2000;
    this.bobLp2Balance = 0;
    this.carlLp2Balance = 1600;

    assert.equal(this.aliceLp2Balance, balanceAlice);
    assert.equal(this.bobLp2Balance, balanceBob);
    assert.equal(this.carlLp2Balance, balanceCarl);
  });
  ...
  describe("when created", () => {
    ...
    it("is configured to reward 100 MOCK per block", async () => {
      this.rewardsPerBlock = 100;
      await this.farm.changeRewardPerSecond(this.rewardsPerBlock);
      const rewardPerBlock = await this.farm.rewardPerSecond();
      assert.equal(rewardPerBlock, this.rewardsPerBlock);
    });
  ...
  describe("before the start block", () => {
    before(async () => {
      await Promise.all([
        this.lp.approve(this.farm.address, 3000, { from: alice }),
        this.lp.approve(this.farm.address, 1000, { from: bob }),
      ]);
      this.aliceLPDeposited = 1500;
      this.bobLPDeposited = 500;
      this.aliceLpBalance -= 3000;
      this.bobLpBalance -= 1000;
      await Promise.all([
  ...
  describe("after 10 seconds of farming", () => {
    ...
    it("has a total reward of 1000 MOCK pending", async () => {
      ...
      assert.equal(100 * deltaTime, this.totalPending);
    });

    it("reserved the right amount of rewards for alice and bob", async () => {
      const pendingAlice = await this.farm.pending(0, alice);
      assert.equal(this.totalPending * this.aliceLPDeposited / (this.aliceLPDeposited + this.bobLPDeposited), pendingAlice);

      const pendingBob = await this.farm.pending(0, bob);
      assert.equal(this.totalPending * this.bobLPDeposited / (this.aliceLPDeposited + this.bobLPDeposited), pendingBob);
    });
  });
  ...
  describe("with a 3th participant after 30 seconds", () => {
    before(async () => {
      ...
      await this.lp.approve(this.farm.address, 4000, { from: carl });
  ...
  describe("with a participant withdrawing after 70 seconds", () => {
    ...
    it("gives alice 3755 MOCK and 1500 LP", async () => {
      ...
      assert.equal(7000+750, balanceLP);
    });
